...
func _build_materials() -> void:
	_mat_player = _make_material(Color(0.15, 0.55, 1.0), 0.18, 0.35)
...
func _build_player() -> void:
	...
	sphere.height = PLAYER_RADIUS * 2.0
	sphere.radial_segments = 32
	sphere.rings = 16

	_player = MeshInstance3D.new()
	_player.name = "PlayerSphere"
	_player.mesh = sphere
	_player.material_override = _mat_player
	_player.position = Vector3(_lane_positions[_current_lane], PLAYER_RADIUS, PLAYER_Z)
	add_child(_player)
...
func _restart() -> void:
	...
	_player.position = Vector3(_target_x, PLAYER_RADIUS, PLAYER_Z)
...
func _move_player(delta: float) -> void:
	...
	_player.rotate_x(-WORLD_SPEED * delta / PLAYER_RADIUS)
